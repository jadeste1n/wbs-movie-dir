...
window.addEventListener('load', async () => {
  ...
  searchButton.addEventListener ("click", async function (event) { // add the async to allow await
    ...
    productContainer.appendChild(searchContainer);
      ...
      if (userInput !== '') {
        try {
          ...
          const movies = await fetchMovieSearched(); //function only resumes with rest of code if the promise settles
