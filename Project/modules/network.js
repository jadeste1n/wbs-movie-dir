...
//Fetch for search
  export async function fetchMovieSearched(userInput) { // userInput has to be passed to function to manipulate url
    try {
  ...
  const response = await fetch(`https://api.themoviedb.org/3/search/movie?query=${userInput}&include_adult=false&language=en-US&page=1`, options) // add & after ${userInput} if you use url like: &include_adult=false&language=en-US&
